...
export class ChatController implements IChatController {
  ...
  constructor(
    @inject("IGetUserContactsUsecase")
    private getUserContactsUsecae: IGetUserContactsUsecase,
    @inject("IGetConversationsUsecase")
    private getConversationUsecase: IGetConversationsUsecase,
    @inject("ISendMessageUsecase")
    private sendMessageUsecase: ISendMessageUsecase,
    @inject("IGetMessageUsecase")
    private getMessagesUsecase: IGetMessageUsecase,
    @inject("IUpdateUserOnlineStatusUsecase")
    private updateUserOnlineStatus: IUpdateUserOnlineStatusUsecase,
    @inject("IUpdateLastSeenUsecase")
    private updateLastSeenUsecase: IUpdateLastSeenUsecase,
    @inject("IUploadMediaChatUsecase")
    private uploadMediaUsecase: IUploadMediaChatUsecase,
    ...
    this.io.on("connection", async (socket) => {
        ...
        async ({
          ...
          try {
            ...
            this.io
              ...
              const newNotification = await this.notificationUsecase.sendNotification({
                message: `${recipentName} send a message`,
                receiverId: recipentId,
              })
              this.io?.to(recipentId.toString()).emit("new_message_notification", newNotification);
          } catch (error) {
            console.log(error);
          }
        }
      ...
      socket.on("get_contacts", async ({ userId, userType }) => {
        try {
          ...
          const contacts = await this.getUserContactsUsecae.execute(
            userId,
            userType
          );
      ...
      socket.on("get_conversations", async ({ userId, userType }) => {
        try {
          ...
          const conversations = await this.getConversationUsecase.execute(
            userId,
            userType
          );
      ...
      socket.on('createBooking' , async ({bookingData})=> {
        logger.info("create booking triggered")
        console.log(bookingData)
        const notification = await this.notificationUsecase.sendNotification({
          message: 'New Booking Created',
          receiverId: bookingData.vendorId,
        })
        this.io?.to(bookingData.vendorId).emit('new_message_notification' , notification)
      })
    });
  }
  ...
  async uploadMedia(req: Request, res: Response): Promise<void> {
    ...
    const response = await this.uploadMediaUsecase.execute(
      req.file!,
      conversationId
    );
