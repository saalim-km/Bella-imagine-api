...
import { CreateNewCategoryInput, GetCategoriesFilterInput, getCatJoinRequestInput, UpdateCategory, UupdateCatReqInput } from "../../domain/interfaces/usecase/types/admin.types";
...
export class CategoryManagementUsecase implements ICategoryManagementUsecase {
    constructor(
        ...
        @inject('ICategoryRequestRepository') private _categoryRequestRepository : ICategoryRequestRepository
    ){}

    async getCategories(input: GetCategoriesFilterInput): Promise<PaginatedResponse<ICategory>> {
        ...
        return await this._categoryRepository.getAllCategories({filter : search , limit : input.limit , skip : skip})
    }
    ...
    async updateCategoryStatus(categoryId: Types.ObjectId): Promise<void> {
        ...
        await this._categoryRepository.update(categoryId,{status : !isCatExists.status})
    }
