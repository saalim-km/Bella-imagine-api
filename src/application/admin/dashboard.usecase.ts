        ...
        dashBoardStats.recentUsers = await Promise.all(
            dashBoardStats.recentUsers.map(async(user)=> {
                if(user.profileImage){
                    const isFileExistsInS3 = await this._awsS3Service.isFileAvailableInAwsBucket(user.profileImage)
                    if(isFileExistsInS3){
                        user.profileImage= await this._presignedUrl.getPresignedUrl(user.profileImage)
                    }
                }
        ...
        dashBoardStats.recentPosts = await Promise.all(
            dashBoardStats.recentPosts.map(async(post)=> {
                if(post.userId.profileImage){
                    const isFileExistsInS3 = await this._awsS3Service.isFileAvailableInAwsBucket(post.userId.profileImage)
                    if(isFileExistsInS3){
