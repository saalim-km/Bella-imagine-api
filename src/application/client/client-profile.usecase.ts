...
export class ClientProfileUsecase implements IClientProfileUsecase {
    ...
    async updateClientProfile(input: UpdateClientProfileInput): Promise<IClient> {
        ...
        if(profileImage){
            const isFileExists = await this._awsService.isFileAvailableInAwsBucket(client.profileImage);
            if(isFileExists){
                await this._awsService.deleteFileFromAws(client.profileImage);        
            }
            
            const fileKey = `${config.s3.profile}/${client._id}/${Date.now()}${path.extname(profileImage.originalname)}`;
        ...
        const updatedClient = await this._clientRepository.update(client._id , dataToUpdate)
        ...
        if(updatedClient.profileImage) {
            updatedClient.profileImage = await this._presignedUrl.getPresignedUrl(updatedClient.profileImage)
        }
