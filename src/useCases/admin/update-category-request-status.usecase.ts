  ...
  implements IUpdateCategoryRequestStatusUsecase
{
  ...
  async execute(vendorId: ObjectId, categoryId: ObjectId , status : TCategoryRequestStatus): Promise<void> {
    ...
    const newCategories: ObjectId[] = vendor?.categories ? [...vendor.categories, categoryId] : [categoryId];

    await Promise.all([
        this.categoryRequestRepository.findByIdAndUpdateStatus(
            categoryRequest._id,
            status
        ),
        this.vendorRepository.findByIdAndUpdateVendorCategories(vendorId,newCategories),
        this.notificationRepository.save({message : `Your category join request has been ${status}, for any enquery please contact Bella Imagine Team.`,receiverId : vendorId.toString()})
    ])
  }
}
